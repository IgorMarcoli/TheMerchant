    ...
    <div class="rounded-3xl bg-slate-900 border border-slate-800 p-8 shadow-2xl">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required autofocus class="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-indigo-500">
            </div>
            ...
            <div class="flex items-center justify-between text-xs">
                <label class="flex items-center gap-2 text-slate-400">
                    <input type="checkbox" name="remember" class="rounded bg-slate-950 border-slate-700 text-indigo-600">
                    Lembrar-me
                </label>
            </div>
        ...
        <p class="text-center text-xs text-slate-400 mt-6">
            Não possui uma conta? <a href="{{ route('register') }}" class="text-indigo-400 font-bold hover:underline">Cadastre-se</a>
        </p>
    </div>
</div>
@endsection
